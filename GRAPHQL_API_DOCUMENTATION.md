...
- **Admins MUST login with OTP** (same flow as regular users)
- All admin operations require JWT token in `Authorization` header
- Use **UPPERCASE** enum values: `ADMIN`, `USER`, `ACTIVE`, `SUSPENDED`, `APPROVE`, `REJECT`

### 🔐 Admin Login Flow

Admins follow the SAME login process as regular users:

1. **Login** → Get OTP sent to email
2. **Check email** → Get 6-digit code
3. **Verify OTP** → Get JWT token with admin role
4. **Use token** → Add to Authorization header

```bash
# Step 1: Login
curl -H "Content-Type: application/json" \
  -d '{"query":"mutation { login(email:\"admin@example.com\", password:\"YourPassword\") { message } }"}' \
  http://localhost:4000/api/graphql
...
# Step 3: Verify OTP
curl -H "Content-Type: application/json" \
  -d '{"query":"mutation { verifyOtp(code:\"123456\") { user { role } token refreshToken } }"}' \
  http://localhost:4000/api/graphql

# Step 4: Use the token from step 3 in all admin requests
```
...
### User Roles (use lowercase in GraphQL)
- `user` - Regular user
- `admin` - Administrator
- `super_admin` - Super administrator

### User Status (use lowercase in GraphQL)
...
### Deletion Actions
- `approve` - Approve deletion request
- `reject` - Reject deletion request
...
**Cause:** Using uppercase enum values in GraphQL
...
**Solution:** Always use lowercase enum values with underscores

### "Insufficient permissions"
**Cause:** User doesn't have admin/super_admin role

**Solution:** Login with admin account credentials
